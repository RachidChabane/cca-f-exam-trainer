 ...
 * *why* and *how* this trainer mirrors the real CCA-F exam, and is honest about
 * what is officially confirmed versus what we inferred. Authored bilingually with
 * the same `fr: typeof en` parity guarantee as the UI dictionary, so the two
...
const en: AboutContent = {
  ...
  lead: 'The Claude Certified Architect — Foundations (CCA-F) exam does not ask trivia. It drops you inside a production system and asks you to make the right architectural call — over and over, about the same system. This trainer is built around that exact shape. Here is how the real exam works, how its scenarios connect to their questions, and the precise choices we made to reproduce it — with sources, and an honest line between what is confirmed and what we inferred.',
...
A practice sitting reproduces the real draw: it picks **4 of the 6 themes at random**, one instance of each, and presents each scenario's questions as a contiguous block while the **scenario context stays pinned beside every question** in the set. The table below maps each real-exam behaviour to the choice we made to reproduce it.`,
  ...
  inferred: [
    ...
    'The uniform 4/3/3/3/2 per-scenario domain split — our choice so any 4-of-6 draw lands on the official weights (documented in blueprint.json).',
  ...
  sources: [
    {
      label: 'Anthropic — Claude Partner Network',
...
Une session d'entraînement reproduit le tirage réel : elle choisit **4 des 6 thèmes au hasard**, une instance de chacun, et présente les questions de chaque scénario en bloc contigu tandis que le **contexte du scénario reste épinglé à côté de chaque question** du jeu. Le tableau ci-dessous relie chaque comportement du vrai examen au choix que nous avons fait pour le reproduire.`,
  ...
  inferred: [
    ...
    'La répartition uniforme 4/3/3/3/2 par scénario — notre choix pour que tout tirage 4 sur 6 retombe sur les pondérations officielles (documenté dans blueprint.json).',
